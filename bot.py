...
- Оценивай СУТЬ ответа, а не его длину или количество примеров
- Если сотрудник правильно понимает концепцию и отвечает по существу — это хороший ответ
- НЕ снижай балл за отсутствие примеров, если вопрос их не требует
- НЕ снижай балл за краткость, если суть раскрыта верно
- НЕ требуй академической полноты — это практическая аттестация продавцов
- Балл 75-85 — это хороший ответ, когда основное понято верно
- Балл ниже 60 только если сотрудник явно не понимает тему или отвечает не по существу

Верни результат строго в формате JSON (без markdown, только чистый JSON):
{{
  "score": <число от 0 до 100>,
  "strengths": "<что сотрудник знает и понимает хорошо, 1-2 предложения>",
  "weaknesses": "<чего не хватает в ответе, 1-2 предложения, или пустая строка если ответ хороший>",
